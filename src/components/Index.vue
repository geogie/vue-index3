                  ...
                  <div class="itemBox">
                    ...
                    <div class="addCartBox flex-between">
                      ...
                      <img src="../../static/img/icon/shop_addCart.png"
                           @click="onAddCart(goodsItem,goodsItem.GoodsName)" v-show="!goodsItem.shopAddCart"/>
                      <img src="../../static/img/icon/shop_addCart_select.png" @click="onAddCart(goodsItem.GoodsName)"
                           v-show="goodsItem.shopAddCart"/>
                    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onAddCart(item, name) {
        console.log("item:" + item + " name:" + name + " carts:" + this.$store.state.carts)
        if (!this.$store.state.carts.includes(item)) {
          console.log("item:" + item + " name:" + name + " include")
          if (!this.$refs.footer.showNum) {
            this.cartLength = this.$store.state.carts.length + 1
            console.log("setCarts-cartLength:" + this.cartLength)
            this.$refs.footer.showNum = true
            this.setCarts(item)
          }
        } else {
          console.log("item:" + item + " name:" + name + " noinclude")
